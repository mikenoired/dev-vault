...
- **Редактор кода:** CodeMirror 6 для редактирования сниппетов
...
- **Линтинг:** Biome (frontend), clippy + rustfmt (backend)
...
- Плоский код важнее “умного”: Избегай вложенности if let/match на 3+ уровня. Предпочитай: ранний выход (return, break) и let ... else { return ... }
- Если блок “проверки/фильтра” занимает больше 6–10 строк — вынеси в функцию с говорящим именем
- Ошибки и отсутствие данных — разные вещи: 
  - Option<T> — когда данных может не быть, и это нормально
  - Result<T, E> — когда это ошибка, и её нужно объяснить
  - Не делай unwrap()/expect() в обычном потоке исполнения. Допустимо только: в тестах, в прототипе, в static инициализациях, где падение реально оправдано (и сообщение понятное)
- Меньше аллокаций — больше смысла:
  - Предпочитай &str вместо String, если не нужно владение
  - Используй as_deref() / as_ref() чтобы не клонировать
  - Для формирования строки — сначала попробуй вернуть &'static str, и только в конце to_string()
  - Не делай format! в цикле без нужды. Если это горячий участок — String::with_capacity и push_str
- Итераторы вместо ручных Vec::push, когда это про трансформации:
  - Если логика выглядит как “взяли элементы → отфильтровали → преобразовали → собрали”, пиши это пайплайном: filter_map, filter, map, collect
  - Если читаемость падает (слишком много замыканий/условий) — откатай назад и вынеси шаги в функции
- Табличные правила вместо if/else-ёлок:
  - Если много проверок одного типа (классификация по ключам/паттернам) — используй массив правил: rules.iter().find_map(...)
  - Для разных длин массивов используй срезы: &["a", "b"][..] → &[&str]
- Проверки предусловий держи в начале функции:
  - если нет X — верни пусто/ошибку
  - если селектор не парсится — верни пусто/ошибку
- Константы и кэширование вместо повторной работы:
  - Повторяющиеся литералы/правила выноси в const
  - Дорогие операции (парсинг селектора, regex) — в static через once_cell::sync::Lazy
- Имена должны объяснять “почему”, а не “как”:
  - is_code_container, is_ignored_href, normalize_link — хорошо
  - process2, handleStuff, do_it — плохо
