...
pub async fn install_documentation(
    ...
    match doc_manager.install_documentation_with_progress(&name, progress_tx).await {
        ...
        Err(e) => {
            tracing::error!("✗ [Command] Failed to install documentation '{}': {:?}", name, e);
...
pub async fn update_documentation(
    ...
    tracing::info!("🔄 [Command] update_documentation called for doc_id: {}", doc_id);
    ...
    match doc_manager.update_documentation_with_progress(doc_id, progress_tx).await {
        Ok(doc) => {
            tracing::info!("✓ [Command] Documentation updated successfully: {}", doc.display_name);
            let _ = app.emit("doc-update-complete", &doc);
            Ok(doc)
        }
        Err(e) => {
            tracing::error!("✗ [Command] Failed to update documentation (id={}): {:?}", doc_id, e);
...
pub async fn delete_documentation(
    ...
    tracing::info!("🗑️  [Command] delete_documentation called for doc_id: {}", doc_id);
    let doc_manager = state.doc_manager.lock().await;
    
    match doc_manager.delete_documentation(doc_id).await {
        Ok(()) => {
            tracing::info!("✓ [Command] Documentation deleted successfully (id={})", doc_id);
            Ok(())
        }
        Err(e) => {
            tracing::error!("✗ [Command] Failed to delete documentation (id={}): {:?}", doc_id, e);
            Err(e.to_string())
        }
    }
}
